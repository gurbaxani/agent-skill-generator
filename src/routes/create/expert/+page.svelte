			...
			<div class="cyber-panel flex flex-col gap-4 p-5">
					...
					{#if skillDraft.enableScripts}
						<div class="mt-4 flex flex-col gap-3 border-t border-(--border-default) pt-4">
							...
							>
								...
								<input
									type="file"
									id="expert-script-file-upload-input"
									multiple
									class="hidden"
									onchange={(e) => {
											...
											<div class="flex items-center justify-between gap-2">
												...
												<div class="flex shrink-0 items-center gap-1.5">
													...
													<button
														...
														class="flex items-center justify-center rounded-[2px] p-1 text-(--text-tertiary) transition-colors hover:bg-(--surface-sunken) hover:text-(--secondary) focus:outline-none"
														title="Remove"
													>
				...
				>
					...
					{#if skillDraft.enableAssets}
						<div class="mt-4 flex flex-col gap-3 border-t border-(--border-default) pt-4">
							<span class="text-xs text-(--text-secondary)">Asset Types:</span>
							<div class="flex flex-col gap-2">
							...
							>
								...
								<input
									type="file"
									id="expert-asset-file-upload-input"
									multiple
									class="hidden"
									onchange={(e) => {
										if (e.currentTarget.files && e.currentTarget.files.length > 0) {
											...
											<div class="flex items-center justify-between gap-2">
												...
												<div class="flex shrink-0 items-center gap-1.5">
													...
													<select
														...
														style="color: var(--text-primary); border: 1px solid var(--border-strong); border-radius: 2px; font-family: var(--font-mono);"
														class="bg-transparent px-1.5 py-0.5 text-[10px] focus:outline-none"
													>
		...
		<div class="sticky-column">
			<div class="flex flex-col gap-4">
				...
				<div class="flex flex-col gap-3">
					...
					{#if publishUrl}
						<div class="cyber-panel p-4 text-center mt-3" style="background: var(--surface-sunken); border-color: var(--accent); width: 100%;">
					...
					{:else}
						{#if githubAuth.token}
							<button
								...
								style="background: var(--accent); color: var(--accent-fg);"
								class="flex w-full items-center justify-center gap-2 rounded-[2px] py-3.5 text-xs font-bold tracking-widest uppercase transition-all hover:bg-(--accent-hover) hover:shadow-[0_0_15px_var(--accent-glow)] focus:outline-none disabled:cursor-not-allowed disabled:opacity-40"
							>
								{#if isPublishing}
						...
						{:else}
							...
							>
								<i class="bi bi-github" aria-hidden="true"></i> Login with GitHub to Publish
							</button>
						{/if}
					{/if}
					{#if publishErrorMsg}
						<p class="mt-2 text-center text-xs font-mono text-(--secondary)">[Error] {publishErrorMsg}</p>
					{/if}
				</div>
			</div>
		</div>
