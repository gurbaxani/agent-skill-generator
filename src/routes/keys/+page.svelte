<script lang="ts">
	...
	const providers: ProviderConfig[] = [
		...
		},
		{
			...
			svg: siOpenrouter,
			hasEndpoint: true,
			defaultEndpoint: 'https://openrouter.ai/keys',
			keyRequired: true,
...
<div class="mx-auto max-w-6xl px-6 py-12 lg:px-12">
	<div class="mb-10 flex flex-col gap-2">
		...
		<p style="color: var(--text-secondary);" class="text-base">
			Configure provider credentials. Keys are stored locally in your browser.
		</p>
	</div>
	...
	<div class="grid items-start gap-12 lg:grid-cols-2">
		...
		<section
			...
			class="flex flex-col gap-6 rounded-lg p-6"
		>
			<h2
				style="color: var(--text-primary); border-bottom: 1px solid var(--border-default);"
				class="pb-3 text-xl font-semibold"
			>
				Add Provider Key
			</h2>
			...
			<form onsubmit={handleSave} class="flex flex-col gap-6">
				<div class="flex flex-col gap-2">
					...
					<div class="grid grid-cols-3 gap-3">
						{#each providers as provider (provider.id)}
							<button
								type="button"
								class="flex flex-col items-center justify-center gap-2 rounded-lg border p-3 transition-colors {selectedProvider ===
								provider.id
									? 'border-(--accent) bg-(--accent-glow) text-(--accent)'
									: 'border-(--border-strong) bg-(--surface-sunken) text-(--text-secondary) hover:text-(--text-primary)'}"
								style={selectedProvider === provider.id
									? 'border: 1px solid var(--accent); color: var(--accent); background: var(--accent-subtle);'
				...
				{#if currentProvider.hasEndpoint}
					<div class="flex flex-col gap-2">
						<label for="endpoint" style="color: var(--text-primary);" class="text-sm font-medium"
							>Base URL / Endpoint</label
						>
						<input
							type="text"
							id="endpoint"
							...
							class="w-full rounded-md p-3 text-sm placeholder:text-(--text-tertiary) focus:border-(--accent) focus:ring-1 focus:ring-(--focus-ring) focus:outline-none"
							autocomplete="off"
						/>
					</div>
				{/if}
				...
				<div class="flex flex-col gap-2">
					<label for="model" style="color: var(--text-primary);" class="text-sm font-medium"
						>Model</label
					>
					<input
						type="text"
		...
		>
			...
			<div class="flex flex-col gap-4">
				{#if !userState.hasKeys()}
					<div
						style="background: var(--surface-sunken); border: 1px dashed var(--border-default); color: var(--text-secondary);"
						class="flex flex-col items-center justify-center gap-4 rounded-lg p-12 text-center text-sm"
					>
						...
						{#if showConfirmSkip}
							<div
								class="mt-2 flex flex-col items-center gap-3 rounded border border-(--border-accent) bg-(--surface-base) p-4 shadow-lg"
							>
								<p style="color: var(--text-primary);" class="text-xs font-medium">
									Are you sure? You will need to write everything manually.
								</p>
						...
						{:else}
							<button
								type="button"
								class="mt-2 rounded border border-(--border-strong) bg-transparent px-4 py-2 text-xs font-semibold tracking-wider text-(--text-secondary) uppercase transition-colors hover:border-(--text-primary) hover:text-(--text-primary)"
				...
				{:else}
						...
						>
							<div class="flex items-center gap-4">
								...
								<div class="flex flex-col gap-1">
									<span style="color: var(--text-primary);" class="text-sm font-semibold"
										>{provider.name}</span
									>
									<div class="flex items-center gap-2 text-xs">
										{#if config.model}
											<span
												style="background: var(--surface-sunken); border: 1px solid var(--border-default); color: var(--text-secondary);"
										...
										{/if}
										{#if config.endpoint}
											<span style="color: var(--text-tertiary);" class="max-w-[150px] truncate"
							...
							<div class="flex items-center gap-4">
								<span style="color: var(--text-tertiary);" class="font-mono text-sm">
									{#if config.key}
										••••{config.key.slice(-4)}
									{:else}
					...
					<div class="mt-4 flex justify-end border-t border-(--border-default) pt-4">
						<a
							href="/create"
							class="flex items-center gap-2 rounded-md bg-(--accent) px-6 py-3 text-sm font-semibold transition-colors hover:bg-(--accent-hover)"
							style="color: var(--accent-fg);"
						>
